...
func (a *Actor) exit() {
	tickTimerIds := make([]int32, 0)
	for k, _ := range a.tickTimers {
		tickTimerIds = append(tickTimerIds, k)
	}
	for i := 0; i < len(tickTimerIds); i++ {
		a.UnsetTickTimer(tickTimerIds[i])
	}

	timeoutTimerIds := make([]int32, 0)
	for k, _ := range a.timeoutTimers {
		timeoutTimerIds = append(timeoutTimerIds, k)
	}
	for i := 0; i < len(timeoutTimerIds); i++ {
		a.UnsetTimeoutTimer(timeoutTimerIds[i])
	}
